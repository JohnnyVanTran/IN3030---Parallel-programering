...
class SieveOfEratosthenes {
    ...
    private int[] collectPrimes() {
        ...
        for (int i = start; i <= n; i += 2) if (isPrime(i)) numOfPrimes++;

        int[] primes = new int[numOfPrimes];
        primes[0] = 2;
        int j = 1;

        for (int i = 3; i <= n; i += 2) if (isPrime(i)) primes[j++] = i;
        return primes;
    }
    ...
    private void traverse(int prime) {
        for (int i = prime * prime; i <= n; i += prime * 2) mark(i);
    }
    ...
    private int nextPrime(int prev) {
        for (int i = prev + 2; i <= root; i += 2) if (isPrime(i)) return i;
